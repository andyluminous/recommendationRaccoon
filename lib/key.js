...
class Key {
  ...
  userPassedSet(userId) {
    this.keyArr = [USER, userId, 'passed'];
    return this.joinKey();
  }

  activeItemsZSet() {
    this.keyArr = [ITEM, 'active'];
    return this.joinKey();
  }

  userIntersectionZSet(userId) {
    this.keyArr = [USER, userId, 'recIntersection'];
    return this.joinKey();
  }

  usersForViewDebtSet() {
    this.keyArr = [USER, 'viewDebt'];
    return this.joinKey();
  }
}
